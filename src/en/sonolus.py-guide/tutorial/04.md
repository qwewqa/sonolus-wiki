...
Create `guide/tutorial/stage.py` with this temporary function:
...
Now replace the temporary contents of `guide/tutorial/stage.py` with:
...
The tuple z-index passed to `Sprite.draw` places the judgment line in layer `0`. Additional tuple values can break ties
between sprites within the same layer.
