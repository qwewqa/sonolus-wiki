...
In `guide/lib/note.py`, add an `init_life` helper beside `init_score`. The helper receives the Note archetype so it can
configure that archetype's life changes. A Perfect judgment restores `10` life. A Miss deducts `100`:
...
By default, life starts at its maximum of `1,000`. The `10`-life reward has no effect while life is full, but restores
life after a loss. Reaching `0` marks the level as failed without ending the current play session.
