...
`level_score().update` call in `init_score`: every `10` such judgments adds `0.01` to the multiplier. Only the first
`50` judgments count, so the bonus stops at `0.05`.
...
In the same file, add both layouts to `init_ui` immediately after `ui.combo_text.update(...)`. Stack them so both remain
visible:
