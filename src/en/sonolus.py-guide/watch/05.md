...
`WatchNote` uses the same archetype name as the play-mode Note, so it handles the same entities from the level data.
During replay, that matching name also associates it with the result recorded by the scored play archetype.
...
Now edit `guide/watch/initialization.py`. Add the life helper and `WatchNote`, then call `init_life` between score and
UI setup. The completed file should contain:
...
The [`entity_memory`](https://sonolus.py.qwewqa.xyz/reference/sonolus.script.archetype/#sonolus.script.archetype.entity_memory)
declarations preserve the calculated times for later callbacks. `target_time` is the note's level time.
...
not visible and produces no feedback. The next four chapters add drawing, sound, and particles.
